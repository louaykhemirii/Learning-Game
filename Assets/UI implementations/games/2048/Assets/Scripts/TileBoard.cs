...
public class TileBoard : MonoBehaviour
{
    ...
    private float minimumSwipeDistance = 0.3f; // Adjust this value to control the minimum swipe distance
        ...
        if (!waiting)
        {
            ...
            if (Input.touchCount > 0)
            {
                ...
                else if (touch.phase == TouchPhase.Ended)
                {
                    if (isSwipe)
                    {
                        // Handle the case where a touch was released without performing a swipe
                        // This can be considered as a random touch and can be ignored
                        isSwipe = false;
                    }
                }
            }
